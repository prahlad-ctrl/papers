...
'''
Input Embeddings -
It allows to convert the original sentence into a vector of dim 512 (dmodel). We first convert the sentence into a list of input IDs which is a number, 
And then each of this number corresponds to a vector which is of size 512
'''
...
class InputEmbedding(nn.Module):
    ...
    def forward(self, x):
        return self.embedding(x) * math.sqrt(self.dmodel) # In embedding layers, we multiply those weights by sqrt of dmodel (as per the paper) to stabilizes variance
    
'''
Positional Encoding -
As our input is mapped into embedding of size 512, we want to convey the position of each word in the sentence and therefore add another vector of same size of embedding (size 512)
including special sin, cos function values to show this word occupies this position in the sentence
'''

class PositionalEncoding(nn.Module):
    def __init__(self, seq_len, dropout, dmodel): # added the sequence length and regularization along with dmodel
        super().__init__()
        self.dmodel = dmodel
...
'''
LAYER NORMALIZATION -
For each item in the batch, we calculate the mean and variance and then calculate the new values using their own mean and variance
'''

class LayerNorm(nn.Module):
    def __init__(self, dmodel, epsilon: float=1e-6): #prevents division by zero and numerical stability
        super().__init__()
...
'''
Feed Forward -
Just a simple feed forward (2 layer) network that is used in both encoder and decoder blocks
'''
